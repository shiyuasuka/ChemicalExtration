...
def main(argv: list[str] | None = None) -> None:
    ...
    def _log_summary(summary: dict) -> None:
        if summary.get("success"):
            if summary.get("skipped"):
                ...
                print(f"   G4 Binding Records: {summary.get('g4_bindings', 0)}")
            else:
        ...
        try:
            ...
            if extraction_path.exists() and not args.force_rerun:
                try:
                    ...
                    items = data.get("items") or data.get("g4_bindings", [])
            ...
            items = (
                result.get("final_data", {}).get("records")
                or result.get("final_data", {}).get("items")
                or result.get("final_data", {}).get("g4_bindings", [])
            )
            bindings_count = len(items)
            return {
                "file": file_path.name,
    ...
    if not args.ocr_only and successful > 0:
        ...
        total_bindings = sum(r.get("g4_bindings", 0) for r in results_summary if r["success"])
        print(f"Total G4 binding records: {total_bindings}")
    ...
    for r in results_summary:
        if r["success"]:
            if args.ocr_only:
                ...
                print(f"[SKIPPED] {r['file']}: {r.get('g4_bindings', 0)} records (already processed)")
            else:
                flag_icon = "[FLAGGED]" if r.get("flag") else "[OK]"
                print(f"{flag_icon} {r['file']}: {r.get('g4_bindings', 0)} records")
        else:
