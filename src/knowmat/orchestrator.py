...
def _build_qa_report(base_name: str, final_data: dict, final_state: dict) -> dict:
    ...
    unique_ligands = {
        (record.get("ligand_id") or record.get("ligand_name") or "").strip()
        for record in records
        if (record.get("ligand_id") or record.get("ligand_name"))
    }
    methods = {
    ...
    records_with_value = sum(1 for record in records if record.get("value"))
    records_with_sequence = sum(1 for record in records if record.get("sequence"))
    missing_paper_id = 1 if not final_data.get("paper_id") else 0

    red_line_triggers = []
    if len(records) == 0:
        red_line_triggers.append("NO_RECORDS")
    if records and records_with_value == 0:
        red_line_triggers.append("NO_ENDPOINT_VALUES")
    if records and not methods:
        red_line_triggers.append("NO_METHODS")
