...
class SubFieldDetection(BaseModel):
    ...
    ----------
    sub_field: str
        The niche domain of biochemistry / biophysics that best describes the paper.
        One of ``biophysics``, ``biochemistry``, ``medicinal_chemistry``,
        ``structural_biology``, ``chemical_biology``, ``analytical_chemistry``,
        ``pharmacology``, or ``hybrid``.
    updated_prompt: str
    ...
    sub_field: str = Field(
        description=(
            "The detected G4 ligand binding sub‑field.  Must be one of: biophysics, "
            "biochemistry, medicinal_chemistry, structural_biology, chemical_biology, "
            "analytical_chemistry, pharmacology, or hybrid."
        )
    )
...
# --------------------------------------------------------------------------------
# G4 Ligand Binding models (KnowMat 2.0 new domain)
# --------------------------------------------------------------------------------

class G4BindingItem(BaseModel):
    """Represents one stage-1 core G4 ligand binding record."""

    ligand_name: Optional[str] = Field(
        default=None,
        description=(
            "Ligand name exactly as reported in the paper. Keep the reported alias "
            "string when multiple names are printed together. Do not standardize or "
            "invent identifiers in stage 1."
        ),
    )
    method: Optional[str] = Field(
        default=None,
        description=(
            "Experimental or computational method as reported in the paper. Prefer "
            "canonical names when they are obvious from the source text."
        ),
    )
    value: Optional[str] = Field(
        default=None,
        description=(
            "Raw reported result string exactly as written in the paper. Preserve "
            "units, scientific notation, inequalities, and endpoint labels such as "
            "'Delta Tm2=6.4 ℃' or 'Kd = 2.3 µM'. Do not output pointer strings like "
            "'see supplementary' as values."
        ),
    )
    sequence: Optional[str] = Field(
        default=None,
        description=(
            "Exact nucleotide sequence string when explicitly recoverable. Use null "
            "when the paper does not provide the explicit sequence. Do not place "
            "sequence aliases such as 'c-MYC' or 'ZGQ1' here; those belong in "
            "sequence_name."
        ),
    )
    sequence_name: Optional[str] = Field(
        default=None,
        description=(
            "Sequence alias or context label used in the paper, such as 'c-MYC', "
            "'hTel', or 'Pu24; PDB ID: 2MGN'."
        ),
    )


class G4BindingList(BaseModel):
    """Encapsulates stage-1 core G4 ligand binding records."""

    g4_bindings: List[G4BindingItem] = Field(
        description=(
            "A list of extracted stage-1 core records. Each item should contain only "
            "the five literature-grounded fields: ligand_name, method, value, "
            "sequence, and sequence_name. Split records whenever ligand, sequence, "
            "method, or reported endpoint value changes. Do not add derived or "
            "standardized fields at this stage."
        )
    )
...
class ManagerFeedback(BaseModel):
    ...
    final_extracted_data: G4BindingList = Field(
    ...
    human_review_guide: str = Field(
        description=(
            ...
            "Be specific about ligand names, sequence names, measurement values, or activity labels to verify."
        )
    )
...
extraction_extractor = _LazyExtractor(
    [G4BindingList], "G4BindingList", enable_inserts=False, agent_type="extraction"
)
